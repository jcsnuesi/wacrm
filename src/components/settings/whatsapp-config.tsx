...
export function WhatsAppConfig() {
    ...
    async (acctId: string) => {
      ...
      try {
        ...
        if (!res.ok || !payload) {
          ...
          setResetReason(null);
          setStatusMessage(
            'Health check endpoint returned a non-JSON response.'
          );
          setConfig(null);
          setConfigOptions([]);
        ...
        const options = payload.configs ?? [];
        const selected =
          options.find((row) => row.id === payload.active_config_id) ??
          options.find((row) => row.is_active) ??
          null;

        setConfigOptions(options);
        setActiveConfigId(selected?.id ?? null);
        ...
        if (selected) {
          try {
            const statusRes = await fetch('/api/whatsapp/config', {
              method: 'GET',
            });
            const statusPayload = await safeJson<{
              connected?: boolean;
              needs_reset?: boolean;
              reason?: string;
              message?: string;
            ...
            if (!statusRes.ok || !statusPayload) return;
            ...
            if (statusPayload.connected) {
              ...
              setConnectionStatus('disconnected');
              setResetReason(
                statusPayload.needs_reset
                  ? 'token_corrupted'
                  : statusPayload.reason === 'meta_api_error'
                    ? 'meta_api_error'
                    : null
              );
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (tokenEdited && accessToken !== MASKED_TOKEN && accessToken.trim()) {
        ...
        toast.error(
          'Please enter a valid Access Token for the new WhatsApp line'
        );
        setSaving(false);
        return;
      }
  ...
  async function handleTestConnection() {
    try {
      ...
      if (!res.ok || !payload) {
        setConnectionStatus('disconnected');
        toast.error(
          'Health check endpoint did not return JSON. Verify dev server and session.'
        );
        return;
      }
  ...
  async function handleReset() {
    ...
    try {
      ...
      const url = activeConfigId
        ? `/api/whatsapp/config?config_id=${encodeURIComponent(activeConfigId)}`
        : '/api/whatsapp/config';
  ...
  function handleNewConfig() {
    ...
    setResetReason(null);
    setStatusMessage(
      'Adding a new WhatsApp line. Saving will set it as active.'
    );
    setRegistrationProbe(null);
  }

  async function handleSwitchConfig(nextConfigId: string) {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(data?.error || 'Failed to switch WhatsApp line');
        return;
      }

      toast.success('Active WhatsApp line switched');
      if (accountId) {
        await fetchConfig(accountId);
      }
    } catch (err) {
      console.error('Switch config error:', err);
      toast.error('Failed to switch WhatsApp line');
    } finally {
