...
export async function loadAiConfig(
  ...
  let { data, error } = await selectAiConfig(db, accountId)

  if (error && isMissingOptionalConfigColumn(error)) {
    console.warn(
      `[ai config] optional ai_configs columns are missing for account ${accountId}; falling back to legacy config shape. Apply migrations 030 and 033 to enable embeddings and handoff routing.`,
      error,
    )
    const legacy = await selectAiConfig(db, accountId, LEGACY_CONFIG_COLUMNS)
    data = legacy.data
    error = legacy.error
  }
