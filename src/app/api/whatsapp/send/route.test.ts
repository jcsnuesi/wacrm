...
let providerKind: 'meta' | 'twilio' = 'meta';
        ...
        case 'whatsapp_config':
          return {
            data: {
              ...
              sender_phone: providerKind === 'twilio' ? '+14155550199' : null,
...
const { sendTwilioMessage } = vi.hoisted(() => ({
  sendTwilioMessage: vi.fn(async () => ({
    messageId: 'SMaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
  })),
}));
vi.mock('@/lib/whatsapp/twilio-api', () => ({
  sendTwilioMessage,
  TwilioProviderError: class TwilioProviderError extends Error {},
}));
...
describe('POST /api/whatsapp/send — contact_id template path', () => {
  ...
  it('routes a Twilio conversation through Twilio and never calls Meta', async () => {
    providerKind = 'twilio';
    existingConversation = {
      id: 'conv-existing',
      account_id: 'acct-1',
    ...
    const res = await POST(
      new Request('http://localhost/api/whatsapp/send', {
        ...
        body: JSON.stringify({
          conversation_id: 'conv-existing',
          message_type: 'text',
          content_text: 'Hello from Twilio',
        }),
      })
    );
    const json = await res.json();
    ...
    expect(res.status).toBe(200);
    expect(json.whatsapp_message_id).toBe('SMaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa');
